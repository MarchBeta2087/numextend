...
| v0.16 | 2026-08-13 | Newton 开方 (bigint_bin_sqrt 落地：Newton 迭代 x ← (x + a/x) >> 1，初值 2^ceil(blen/2) 保收敛，整数平方根向下取整)、Newton 除法（整数倒数 + 商修正，正确性验证过但全精度迭代效率不如 BZ，NEX_DIV_NEWTON_PATH 条件编译隐藏的实验路径)；golden 新增 sqrt 命令与单元测试；Burnikel-Ziegler 递归除法缺陷定位并修复（§13 #1，恢复自动分派）：根因是 `bz_mag_cmp` 对未规范化视图（前导零）按长度比较——稀疏除数移位后中间全零，a12 视图顶肢为 0 而实际值 > b，误判走 3a 分支、破坏 d2n1n2 前提、商溢出截断（余数下溢为全 1）；修复为先裁剪前导零再比较；商累加改为块直写（预留 (t−1)n+1 肢，qi 为罕见 n+1 肢时回退累加），a/b=256 时较 Knuth D 快 ~22%（1000 肢除数：830ms vs 1059ms）；稀疏/全 1/2 的幂/负数/非平衡随机黄金对拍全过 |
